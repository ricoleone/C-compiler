...
{
    assert(token_peek_next()->type == TOKEN_IDENTIFIER);
...
{
    struct token *next_token = token_next();
    if (!next_token || next_token->type != TOKEN_KEYWORD || !S_EQ(next_token->sval, keyword))
    {
...
{
    if (token_peek_next()->type == TOKEN_OPERATOR)
    {
...
{
    expect_op("(");
    if (token_peek_next()->type == TOKEN_KEYWORD)
    {
        parse_for_cast();
        return;
    }
...
void parser_ignore_int(struct datatype *dtype)
{
    ...
    {
        compiler_error(current_process, "Secondary \"int\" type - not supported with this current abbrevation\n");
    }
...
void parse_statement(struct history *history)
{
    ...
    parse_expressionable_root(history);
    if (token_peek_next()->type == TOKEN_SYMBOL && !token_is_symbol(token_peek_next(), ';'))
    {
        parse_symbol();
        return;
    }
...
void parser_append_size_for_node(struct history *history, size_t *_variable_size, struct node *node)
{
    ...
    else if (node->type == NODE_VARIABLE_LIST)
    {
...
{
    printf("INSIDE: parse_body_single_statement\n");
    ...
    if (stmt_node->type == NODE_VARIABLE)
    {
...
{
    printf("INDSIDE: parse_body_multiple_statements\n");
    make_body_node(NULL, 0, false, NULL);
    struct node *body_node = node_pop();
    body_node->binded.owner = parser_current_body;
    parser_current_body = body_node;
    ...
    while (!token_next_is_symbol('}'))
    {
        ...
        if (stmt_node->type == NODE_VARIABLE)
        {
            ...
            if (variable_node_is_primative(stmt_node))
            {
...
void parse_body(size_t *variable_size, struct history *history)
{
    ...
    // parse statements between curly brackets, e.g. {int a; int b;}
...
void parse_case(struct history *history)
{
    ...
    if (case_exp_node->type != NODE_NUMBER)
    {
...
int parse(struct compile_process *process)
{
    ...
    parser_blank_node = node_create(&(struct node){.type = NODE_BLANK});
