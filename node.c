...
{
    node_create(&(struct node){.type = NODE_CAST, .cast.dtype = *dtype, .cast.operand = operand_node});
}

void make_tenary_node(struct node *true_node, struct node *false_node)
{
    node_create(&(struct node){.type = NODE_TENARY, .tenary.true_node = true_node, .tenary.false_node = false_node});
}

void make_case_node(struct node *exp_node)
{
    node_create(&(struct node){.type = NODE_STATEMENT_CASE, .stmt._case.exp = exp_node});
}


void make_goto_node(struct node *label_node)
{
    node_create(&(struct node){.type = NODE_STATEMENT_GOTO, .stmt._goto.label = label_node});
}

void make_label_node(struct node *name_node)
{
    node_create(&(struct node){.type = NODE_LABEL, .label.name = name_node});
}

void make_continue_node()
{
    node_create(&(struct node){.type = NODE_STATEMENT_CONTINUE});
}

void make_break_node()
{
    node_create(&(struct node){.type = NODE_STATEMENT_BREAK});
}
...
void make_exp_node(struct node *left_node, struct node *right_node, const char *op)
{
    assert(left_node);
    assert(right_node);
    node_create(&(struct node){.type = NODE_EXPRESSION, .exp.left = left_node, .exp.right = right_node, .exp.op = op});
}

void make_exp_parentheses_node(struct node *exp_node)
{
    node_create(&(struct node){.type = NODE_EXPRESSION_PARENTHESES, .parenthesis.exp = exp_node});
}

void make_bracket_node(struct node *node)
{
    node_create(&(struct node){.type = NODE_BRACKET, .bracket.inner = node});
}
...
void make_union_node(const char *name, struct node *body_node)
{
    ...
    node_create(&(struct node){.type = NODE_UNION, ._union.body_n = body_node, ._union.name = name, .flags = flags});
}
...
{
    node_create(&(struct node){.type = NODE_STATEMENT_DO_WHILE, .stmt.do_while_stmt.body_node = body_node, .stmt.do_while_stmt.exp_node = exp_node});
}

void make_while_node(struct node *exp_node, struct node *body_node)
{
    node_create(&(struct node){.type = NODE_STATEMENT_WHILE, .stmt.while_stmt.exp_node = exp_node, .stmt.while_stmt.body_node = body_node});
}

void make_for_node(struct node *init_node, struct node *cond_node, struct node *loop_node, struct node *body_node)
{
    node_create(&(struct node){.type = NODE_STATEMENT_FOR, .stmt.for_stmt.init_node = init_node, .stmt.for_stmt.cond_node = cond_node, .stmt.for_stmt.loop_node = loop_node, .stmt.for_stmt.body_node = body_node});
}

void make_return_node(struct node *exp_node)
{
    node_create(&(struct node){.type = NODE_STATEMENT_RETURN, .stmt.return_stmt.exp = exp_node});
}

void make_else_node(struct node *body_node)
{
    node_create(&(struct node){.type = NODE_STATEMENT_ELSE, .stmt.else_stmt.body_node = body_node});
}

void make_if_node(struct node *cond_node, struct node *body_node, struct node *next_node)
{
    node_create(&(struct node){.type = NODE_STATEMENT_IF, .stmt.if_stmt.cond_node = cond_node, .stmt.if_stmt.body_node = body_node, .stmt.if_stmt.next = next_node});
}
...
{
    return node->type == NODE_EXPRESSION_PARENTHESES || node->type == NODE_EXPRESSION;
}

bool node_is_value_type(struct node *node)
{
    return node_is_expression_or_parentheses(node) || node->type == NODE_IDENTIFIER || node->type == NODE_NUMBER || node->type == NODE_UNARY || node->type == NODE_TENARY || node->type == NODE_STRING;
}


bool node_is_expression(struct node *node, const char *op)
{
    return node->type == NODE_EXPRESSION && S_EQ(node->exp.op, op);
}
